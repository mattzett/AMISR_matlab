...
%
% Usage:
%   [vest,Pvest] = vfield_holistic_differential(vlos,dvlos,xgmag,ygmag,K,Nx,Ny,alpha)
%
% Inputs:
...
% 6 Aug 2009
%
% NOTE: This is not the approach I will end up using.  Instead, see Sulzer,
% et. al (2005) for a detailed example how to apply linear regularization
% to ISR vector velocity estimation.  T.B. 21 Aug 2009


function [vest,Pvest,x2,y2] = vfield_holistic_differential(vlos,dvlos,xgmag,ygmag,K,Nx,Ny,alpha)

debugging = 0; % Show debug figures etc.
...
vest = Pv*A'*pinv(A*Pv*A' + alpha^2*D*Pv*D' +Pe) * vlos;
Pvest = Pv - Pv*A'*pinv(A*Pv*A' + alpha^2*D*Pv*D' + Pe)*A*Pv;
...
Pvyz = Pvest(2*Ny*Nx+(1:Ny*Nx), 2*Ny*Nx+(1:Ny*Nx));
