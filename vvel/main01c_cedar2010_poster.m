...
figure(100); % vx and vy together
plot_idx = 1;
for y = Ny:-1:1,
    for x = 1:Nx,
        subplot(Ny,Nx,plot_idx),
        h(1) = errorbar( mtime(1,tstart:tend), squeeze(VX(y,x,tstart:tend)), squeeze(DVX(y,x,tstart:tend)), 'b-' );
        hold on;
        h(2) = errorbar( mtime(1,tstart:tend), squeeze(VY(y,x,tstart:tend)), squeeze(DVY(y,x,tstart:tend)), 'Color', [0,0.5,0] );
        hold off;
        datetick('x','HHMM')
        if y == 1,
            xlabel('UT');
...
legend(h,{'Zonal','Meridional'},'Location','SouthEast')
set(gcf,'PaperSize',[14,10]);
set(gcf,'PaperPosition',[0,0,14,10]);
print('-f100','-depsc2','-r200',fullfile(outputdir,'history_vxvy.eps'));
