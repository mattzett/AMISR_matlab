%-------------------------------------------------------------------------%
% Test 4: Bayesian velocity estimator.
% 
% Thomas Butler
% 26 May 2009
%
...
range_idx = find( Alt>200e3 & Alt<400e3 );
el2 = el1(range_idx);
az2 = az1(range_idx);
kx = cos(el2) .* sin(az2);
ky = cos(el2) .* cos(az2);
...
% Right now, this line is only valid for Nov 11 2007 unwarped allsky data
% (T.B., 24 July 2009)
% itime = itime_all;
...
% Use these 'lims' for November 11, 2007.
%     lims = [-100,100,-50,125]*1e3;
...
%         image_disp = image_disp.^(1 - 0.4); % Brighten the image
%         imshow(image_disp,'XData',xp/1e3,'YData',yp/1e3);
...
%         axis equal
        xlabel('Ground distance east (km)','FontSize',14);
        ylabel('Ground distance north (km)','FontSize',14);
...
for y = Ny:-1:1,
    for x = 1:Nx,
        ...
        title(sprintf('(%d, %d)', x, y));
        axis([mtime(1,tstart),mtime(1,tend),-50,50]);
